...
Это тот шов, вдоль которого отделяется стенограф: отдельный процесс держит
микрофон и распознаёт, а страница становится просто окном и микрофон не
запрашивает. Имя источника доходит до строки в ленте — при двух входах по нему
можно будет различать говорящих.
